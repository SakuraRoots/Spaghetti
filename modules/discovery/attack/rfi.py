...
class Rfi:
    ...
    def run(self):
        ...
        dbfiles = read_data_file('data/rfi.txt')
                    ...
                    if len(param) > 1:
                        for para in param:
                            ...
                            if re.search(pl, resp.content):
                                self.output.plus(
                                    'That site is may be vulnerable to Remote File Inclusion (RFI) at %s' % para)
                        ...
                        if re.search(pl, resp.content):
                            self.output.plus(
                                'That site is may be vulnerable to Remote File Inclusion (RFI) at %s' % param[0])
        except Exception:
            pass
