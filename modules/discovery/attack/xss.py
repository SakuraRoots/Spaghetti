...
class Xss:
    ...
    def run(self):
        ...
        dbfiles = read_data_file('data/xss.txt')
                ...
                for url in self.urls:
                    ...
                    if len(param) > 1:
                        for para in param:
                            ...
                            if resp.status_code == 200:
                                if re.search(payload[0], resp.content, re.I):
                                    self.output.plus(
                                        'That site is may be vulnerable to Cross Site Scripting (XSS) at %s' % para)
                        ...
                        if resp.status_code == 200:
                            if re.search(payload[0], resp.content, re.I):
                                self.output.plus(
                                    'That site is may be vulnerable to Cross Site Scripting (XSS) at %s' % param[0])
        except Exception:
            pass
